...
// ===========================================
// DÉSACTIVÉ — REBUILD.md T0.6.
// Cette route créait un compte super_admin sans aucune authentification ;
// le seul garde-fou était « un admin existe déjà ». À remplacer par une
// commande CLI protégée en phase 3 (T3.15).
app.post('/api/admin/setup', async (_req, res) => {
  return res.status(410).json({
    error: "Route désactivée. Créer un administrateur via la commande CLI (voir REBUILD.md T3.15).",
  });
});
...
async function executeSearchProducts(params: {
  ...
  const query = `
    SELECT 
      ...
      b.name as brand_name, b.slug as brand_slug, b.city as brand_city,
    ...
    WHERE ${whereClause}
    ORDER BY 
      CASE WHEN p.name ILIKE '%${keywords[0] || ''}%' THEN 0 ELSE 1 END,
      p.price_min ASC
    LIMIT ${limit}
  `;
...
const CHAT_SYSTEM_PROMPT = `Tu es l'assistant shopping Made in France.

INSTRUCTION CRITIQUE - À SUIVRE OBLIGATOIREMENT :
Tu dois TOUJOURS appeler un outil (search_products ou search_brands) AVANT de répondre.
Ne réponds JAMAIS sans avoir d'abord fait une recherche dans la base de données.
...
PROCESSUS OBLIGATOIRE pour chaque message :
1. APPELLE search_products avec query correspondant à la demande
2. SI pertinent, appelle aussi search_brands
3. Présente les résultats de façon concise (2-3 phrases max)
4. Termine par des suggestions

Exemples de recherches :
- "un pull" → search_products(query="pull", sector="Mode & Accessoires")
- "cadeau 50€" → search_products(query="cadeau", max_price=50)
- "marque de sacs" → search_brands(query="sac", sector="Mode & Accessoires")
- "cosmétiques bio" → search_products(query="cosmétique bio", sector="Beauté & Bien-être")

Style : tutoiement, quelques emojis (🇫🇷 ✨), enthousiaste mais bref.
...
Exemples de suggestions : "Pour homme|Pour femme|Moins de 50€|Style classique"

RAPPEL : Ne jamais inventer de produits. Utilise UNIQUEMENT les résultats des outils.`;
...
// Les clés d'API vivent uniquement dans l'environnement du serveur (CLAUDE.md, règle 4).
// Elles ne sont ni renvoyées dans une réponse HTTP, ni stockées en base.
...
app.put('/api/admin/ai/settings', async (req, res) => {
  try {
    // Les clés éventuellement envoyées par le client sont ignorées, pas persistées.
