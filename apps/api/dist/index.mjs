...
var stripeClient = () => new Stripe(process.env.STRIPE_SECRET_KEY, { apiVersion: STRIPE_API_VERSION });
...
var limiterRecherche = rateLimit({
  windowMs: 6e4,
  limit: 30,
  standardHeaders: "draft-7",
  legacyHeaders: false,
...
app.get("/api/v1/products", async (req, res) => {
  try {
    ...
    if (query.trim()) {
      ...
      const filters = [Prisma.sql`p.status = 'ACTIVE'`];
      ...
      const where2 = Prisma.sql`${Prisma.join([...filters, matches], " AND ")}`;
      const orderBy2 = sort === "price-asc" ? Prisma.sql`p.price_min ASC NULLS LAST` : sort === "price-desc" ? Prisma.sql`p.price_min DESC NULLS LAST` : sort === "name-asc" ? Prisma.sql`p.name ASC` : Prisma.sql`similarity DESC, p.name ASC`;
...
async function executeSearchBrands(params) {
  ...
  const sector = params.sector?.replace(/&amp;/g, "&");
  const conditions = [Prisma.sql`b.status = 'ACTIVE'`];
  if (sector) conditions.push(Prisma.sql`s.name = ${sector}`);
