...
/**
 * Rendu de la fiche marque.
 *
 * ⚠️ Ce composant reste `'use client'` — il porte une visionneuse d'images, un
 * bouton favori et des sections dépliables. Mais **il ne va plus chercher ses
 * données** : elles arrivent en props depuis le Server Component voisin.
 *
 * C'est ce qui change tout pour le référencement. Auparavant la marque était
 * chargée dans un `useEffect` : le HTML servi ne contenait qu'un « Chargement…
 * ». Un composant client reçoit son rendu initial côté serveur ; à condition
 * que les données soient déjà là.
 */
export default function BrandDetail({ brand, similarBrands, products }: BrandDetailProps) {
