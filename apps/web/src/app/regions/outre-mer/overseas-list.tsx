...
const DOMTOM_REGIONS = [
  ...
  // La Polynésie et la Nouvelle-Calédonie manquaient : les marques qui s'y
  // trouvent n'apparaissaient nulle part.
  { slug: 'polynesie-francaise', name: 'Polynésie française', color: '#0284C7' },
  { slug: 'nouvelle-caledonie', name: 'Nouvelle-Calédonie', color: '#EA580C' },
];
...
export interface OverseasListProps {
  allBrands: Brand[];
 ...
 * Marques d'outre-mer.
 *
 * ⚠️ Cette page faisait **cinq appels HTTP séquentiels** — un par territoire —
 * a chaque changement de page ou de filtre, puis paginait cote client. Et elle
 * n'en listait que cinq : la Polynésie et la Nouvelle-Calédonie manquaient.
 *
 * Tout arrive desormais en une requete, cote serveur.
 */
export default function OverseasList({ allBrands, regionCounts }: OverseasListProps) {
  const [currentPage, setCurrentPage] = useState(1);
  const [selectedTerritory, setSelectedTerritory] = useState<string | null>(null);

  const filtered = selectedTerritory
    ? allBrands.filter((b) => b.region === regionCounts.find((r) => r.slug === selectedTerritory)?.name)
    : allBrands;

  const totalBrands = regionCounts.reduce((sum, r) => sum + r.count, 0);
  const totalPages = Math.max(1, Math.ceil(filtered.length / ITEMS_PER_PAGE));
  const brands = filtered.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE);
