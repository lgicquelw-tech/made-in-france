...
beforeEach(async () => {
  ...
  const m = await prisma.brand.create({ data: { name: 'TRICOTS DE QUIMPER', slug: 'tricots-quimper', status: 'PENDING_REVIEW', sectorId: mode.id, regionId: bretagne.id, city: 'Quimper' } });
...
describe('repondre', () => {
  test('deux outils dans le meme tour : deux tool_result dans UN message, produits ET marques renvoyes', async () => {
    ...
    expect(r.brands.map((b) => b.name)).toEqual(['TRICOTS DE QUIMPER']); // PENDING_REVIEW : visible
