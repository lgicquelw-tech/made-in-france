...
pnpm build                # PASSE : 970 pages en 20 s contre la vraie base (18 septembre 2026).
                          # Les fiches produit ne sont PAS prerendues (35 000) : rendues a la demande, cache 1 h.
                          # Ne jamais exporter .env dans le shell avant : NODE_ENV=development casse le build.
...
pnpm data:geocode --suggerer         # ... et propose une commune pour les non placees, sans rien ecrire
...
pnpm test:e2e                        # 36 parcours Playwright, serveur Next lance sur madeinfrance_test
```
...
| **Build de production** | `produits/[slug]` ne prérend **aucune** fiche (`generateStaticParams` → `[]`) : 35 000 pages à chaque déploiement, c'est une heure de build contre une base distante. `dynamicParams` + `revalidate` font le même travail à la première visite (163 ms, puis 3 ms). Et **`NODE_ENV=development` exporté dans le shell avant `next build`** produit `Cannot read properties of null (reading 'useContext')` sur toutes les pages : ne jamais `source .env` avant un build ; `NODE_ENV` n'est plus dans `.env.example`. |
...
| **Double appel au montage** | Le motif `if (!hydrated) { setHydrated(true); return; }` avec `hydrated` dans les dépendances relance l'effet et refait l'appel que le serveur venait de rendre. Corrigé dans cinq composants ; utiliser une référence sur la dernière requête résolue, jamais ce drapeau. Variante Studio : un effet sur `[slug, status]` charge à `loading` **et** à `authenticated` — **trois** appels mesurés au lieu d'un, et le dernier écrase la saisie en cours. Corrigé sur les six pages du Studio (`if (status === 'loading') return;`) ; un parcours compte les appels. |
| Communes fusionnées | Six fiches portaient le nom d'une commune disparue (Doué-la-Fontaine, Montjean-sur-Loire, Saint-Pierre-Montlimart…) ou d'un village rattaché (Puyricard, Arèches). Corrigées dans `data/brands.xlsx` le 18 septembre 2026 sous la forme **« Commune officielle (ancien nom) »** : le géocodage lit la partie avant la parenthèse, la fiche garde le lieu connu. **Corriger en base ne sert à rien** — `pnpm bootstrap` réimporte le fichier et écrase. Après écriture d'un XLSX, differ cellule par cellule contre l'original : la bibliothèque réécrit tout le classeur. |
| Géocodage | Par **commune**, via `api-adresse.data.gouv.fr` ; précision = centre de la commune. Les homonymes (cinq « Saint-Denis ») sont départagés par la région de la marque ; sans correspondance on ne devine pas. **Ne jamais placer une marque depuis un résultat qui n'est pas de type `municipality`** : interrogée sans ce filtre, l'API répond sur une *rue* qui porte le nom cherché, ailleurs — « Monaco » → Sartène, « Île de Ré » → La Rochelle, et « Île de Groix » → un lieu-dit de **Dinan**, dans la bonne région, à 150 km. Mesuré le 18 septembre 2026 : 6 propositions fausses sur 12. `--suggerer` imprime ces pistes sans rien écrire. |
