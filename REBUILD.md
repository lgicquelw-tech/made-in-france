...
| 2026-09-11 | Phase 5 : T5.1 à T5.8 | **Qualité des données, 7 tâches sur 8.** `pnpm data:audit` (lecture seule) : 899 marques publiables sur 903. Liens morts : 37 sites confirmés, désactivation prudente (3 échecs **et** 72 h), URL jamais effacée. Un seul point d'écriture pour les produits scrappés, idempotence prouvée sur une vraie boutique (passage 2 = 0 créé / 10 mis à jour, texte éditorial intact). Publication pilotée par l'audit : 10 produits en ligne avec images, 2 retirés. **Carte : 3 → 868 marques.** Enrichissement IA préparé, **non lancé** (dépense, clé T0.7). **62 tests**, les premiers du projet. Trois défauts sérieux trouvés en chemin : l'import remettait toute marque validée en `PENDING_REVIEW` à chaque `bootstrap` ; le scraper Shopify n'écrivait jamais le lien d'achat ; `--all` WooCommerce ignorait toute marque déjà importée. **Décisions en attente** : les 902 marques `PENDING_REVIEW` servies publiquement ; 35 villes à corriger dans le fichier source ; feu vert T5.7. Détail : `docs/JOURNAL.md`. |
|  |  |  |
