...
- [~] **T6.5** — Parcours navigateur : recherche → marque → produit → clic d'achat ; inscription ; connexion ; favoris ; revendication de marque ; édition Studio ; édition admin. → **9 parcours verts en CI** : chaîne publique complète, JSON-LD, 404, connexion, mauvais mot de passe, favoris, accès admin (ordinaire → 404, admin → 200). **Restent** : inscription, revendication, édition Studio et admin — ces pages appellent encore Express (`API_URL`), à couvrir après T3.4/T3.8.
