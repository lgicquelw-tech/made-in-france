...
- [x] **T3.2** — **Arbre `/api/admin/brands/*` migré** (9 routes) vers des Route Handlers Next.js, tous derrière `requireAdmin` — `DELETE` derrière `requireSuperAdmin`, car il emporte en cascade produits, images, propriétaires et demandes de revendication. Aucun fichier ne dépasse 115 lignes. Routes publiques migrées le 17 septembre 2026 ; Express supprimé.
...
- [x] **T3.4** — Migrer la recherche. → **Fait le 16 septembre 2026.** Trois routes Next (`/api/v1/search/all`, `/brands`, `/products`), six clients en URL relative, quatre routes Express retirées (28 → 24). **Défaut majeur corrigé : 191 marques au nom accentué étaient introuvables sans accent** (`unaccent` des deux côtés). Filtres combinés, tri en liste blanche, 9 défauts en tout. 120 + 31 + 14 tests.
...
- [x] **T3.7** — **Médias** : migrés, avec vérification du type MIME réel et liste blanche de dossiers (T3.20). **Paiements** : migrés, avec la propriété vérifiée et le prix dérivé de la base (T8.5, T8.6). **IA** : les *réglages* sont migrés et ne peuvent plus fuir de clé. → **Chat fait le 17 septembre 2026** : `lib/chat/` + `/api/v1/chat`, Zod, limiteur, modèle simulé en test. Corrigé : la boucle ne traitait qu'un `tool_use` par tour ; l'énumération des secteurs était l'ancienne taxonomie ; `search_brands` ne voyait qu'une marque sur 903.
...
- [x] **T3.8** — Supprimer l'ancien `index.ts` une fois toutes les routes migrées et vérifiées. → **Fait le 17 septembre 2026 — `apps/api` supprimé, 3 821 lignes.** Étape 1 : 22 routes `GET` retirées (14 étaient mortes), 9 portées sur Next. Restent le chat (T3.7) et le webhook Stripe. Corrigé au passage : la fiche publique exposait `stripeCustomerId` ; les brouillons de toute marque étaient publics ; une marque `PENDING_REVIEW` ne pouvait pas se revendiquer.
...
- [x] **T3.10** — Rôles en base : fait (`UserRole`, index sur `role`). La vérification serveur existe (voir T3.11) mais n'est pas encore appliquée aux routes : c'est la migration des Route Handlers qui reste. → **Vérifié par script le 17 septembre 2026** : 58 routes, aucune route admin, écriture Studio ou `/me/*` sans garde ; tous les `DELETE` admin derrière `requireAdmin`, ceux de marque et de label derrière `requireSuperAdmin`.
...
- [x] **T3.14** — Piste d'audit : qui a modifié quoi, quand, sur les fiches marque. → **Fait le 17 septembre 2026** : table `audit_logs`, `lib/audit.ts` (champs modifiés seulement, jamais un secret), écriture et trace dans la **même transaction** sur marques, produits, Studio et palier Stripe. Lecture `GET /api/admin/audit`. 6 + 5 tests.
...
- [x] **T3.16** — **Schéma Zod sur chaque route migrée qui accepte un corps ou une query string.** Les routes qui ne prennent qu'un paramètre de chemin n'en ont pas : vérifié qu'un identifiant malformé y produit une **404**, jamais une 500 — Prisma ne trouve simplement rien. → **Vérifié par script le 17 septembre 2026** : toute route lisant un corps ou une query string passe par Zod, les deux routes d'envoi de fichier comprises.
...
- [x] **T4.1** — **Annuaire des marques en Server Component.** La première page (12 marques), le total réel et les listes de filtres arrivent du serveur ; le composant de liste garde ses filtres et sa pagination. Le HTML servi contient désormais un `<h1>`, « 903 marques françaises référencées » et 12 fiches, au lieu d'un « Chargement… ».
  ⚠️ **Reste à faire :** les changements de filtre passent encore par un appel à l'API plutôt que par l'URL. Faire de chaque combinaison de filtres une page indexable est un chantier distinct — utile pour le référencement, mais ce n'est pas le même travail.
...
- [x] **T4.3** — **Secteurs et régions faits.** La liste des secteurs était **écrite en dur dans deux pages**, en plus du seed, du script d'import et de `sitemap.ts` — c'est la divergence de taxonomie qui avait laissé 687 marques sans secteur. Elle vient désormais de la base. La page d'un secteur plafonnait aussi à 100 marques : « Mode & Accessoires » en compte **263**, dont 163 n'apparaissaient jamais.
  Ce qui reste dans le code est de la **présentation** — une icône et une phrase — et n'a rien à faire en base.
  **Régions** : la page d'une région déduisait son nom **de la première marque renvoyée**. Une région sans marque s'affichait donc sous un nom reconstruit à partir du slug (« Provence Alpes Cote D Azur »), et une région inexistante affichait une page vide au lieu d'une 404. Le nom vient de la base. La carte reste à faire.
...
- [x] **T5.2** — Vérificateur de liens sur `externalBuyUrl` → désactiver automatiquement les 404. Un lien mort coûte plus de confiance qu'un produit manquant. → `pnpm data:links`. Règle prudente : **3 échecs consécutifs ET premier échec vieux de 72 h**, sinon une panne d'hébergeur d'une journée viderait l'annuaire. L'URL n'est jamais effacée ; un lien qui répond de nouveau est réactivé seul. **10 tests** (`pnpm test:links`) — les premiers du projet. Détail : `docs/JOURNAL.md`.
- [x] **T5.3** — Dédoublonner et filtrer le bruit : cartes cadeaux, échantillons, variantes en double, produits de test. → `scripts/catalogue/noise.ts`, module pur, **13 tests**. Principe : en cas de doute, on garde. Sera appelé par le chemin d'écriture unique que T5.5 doit construire. Détail : `docs/JOURNAL.md`.
- [x] **T5.4** — Contrôles qualité sur les marques : URL valide, secteur et région reconnus, doublons, géolocalisation, statut de vérification. → Géolocalisation **3 → 868 / 903** par l'API Adresse nationale, la région départageant les homonymes (`pnpm data:geocode`, 9 tests). URL de suivi nettoyées. **L'import ne réécrit plus le statut** — chaque `pnpm bootstrap` remettait toute marque validée en attente. **35 marques restent à corriger dans `data/brands.xlsx`** (colonne `city` = région, département ou rien) : liste dans `docs/JOURNAL.md`.
...
- [~] **T5.7** — Relancer l'enrichissement IA uniquement sur les champs manquants. → **Préparé, non lancé.** `pnpm data:enrich` : règle « champs manquants seulement » testée, sortie structurée, consigne « ne rien inventer », simulation sans réseau avec estimation de coût (~0,10 $ pour 10 produits en Opus 5). **En attente d'un feu vert** : clé (T0.7), modèle, périmètre. Ancien script OpenAI supprimé. Détail : `docs/JOURNAL.md`.
- [x] **T5.8** — Ne publier que les produits au-dessus du seuil de complétude (`status = ACTIVE` piloté par l'audit). → `pnpm data:publish`, même jeu de contrôles que l'audit, 7 tests, idempotent. Premier passage : 10 publiés, 2 retirés. **Ne touche pas aux marques** : `pnpm data:publish:brands` s'en charge depuis le 18 septembre 2026 (899 validées). Détail : `docs/JOURNAL.md`.
...
### ✅ Décision prise le 18 septembre 2026 : 899 marques validées en bloc

Le constat, découvert le 1er septembre 2026 : 902 marques sur 903 étaient `PENDING_REVIEW`
— valeur d'import correcte, on ne publie pas 903 fiches sans les avoir regardées — et
**servies publiquement quand même**, parce qu'aucune lecture publique ne filtrait sur le
statut. Le catalogue était incohérent avec lui-même, et le statut ne voulait rien dire.

**Tranché par le propriétaire** (option 2 du choix d'origine, l'audit ayant été fait en
phase 5) : `pnpm data:publish:brands` valide toute marque `PENDING_REVIEW` qui passe les
contrôles bloquants de l'audit. **899 validées**, chacune avec sa ligne d'audit. Quatre
restent en attente, faute de région (NANNETTA, RECYCLED BY LISA, WIA) ou d'une description
d'au moins 40 caractères (OBSTINNÉE).

Et la moitié qui manquait : **les onze lectures publiques respectent désormais le statut**
(`lib/marque-publique.ts`). Une fiche en attente reste joignable par son adresse et
revendicable — sinon son propriétaire ne pourrait jamais la réclamer ; une fiche suspendue
ou refusée répond 404. Détail : `docs/JOURNAL.md`.
...
- [x] **T6.2** — Tests unitaires : garde-fous d'auth (un non-admin reçoit bien 403), scoring de recherche, construction des requêtes, logique d'import. → **Gardes faites** : 18 tests sur `guards.ts` + 6 sur `api-response.ts`, vérifiés **par mutation** (trois gardes cassées à la main, chacune attrapée). Recherche : la saisie hostile finit dans les paramètres liés, jamais dans le SQL — le constat n°4 est **prouvé par un test**. Import : `cleanUrl` (émojis), `lireNom` (noms numériques), `slugify`. **107 tests.**
...
- [x] **T6.4** — Installer Playwright. → Chromium, serveur Next lancé sur la base `_test` semée ; `next start` en CI.
- [x] **T6.5** — Parcours navigateur : recherche → marque → produit → clic d'achat ; inscription ; connexion ; favoris ; revendication de marque ; édition Studio ; édition admin. → **24 parcours verts en CI** (17 septembre 2026). Les quatre derniers vérifient la **base**, pas l'écran : une revendication laisse une demande `PENDING` et zéro propriétaire ; une édition laisse une ligne d'audit. Ils ont trouvé trois défauts du chemin d'édition (double chargement qui écrasait la saisie, cache d'une heure jamais rafraîchi, description courte invisible) — détail dans `docs/JOURNAL.md`.
...
- [~] **T7.5** — **Conformité** — non négociable pour un site français avec comptes et analytics : politique de confidentialité, mentions légales, bandeau de consentement réellement bloquant pour PostHog, suppression de compte et export des données, et mention claire sur l'origine des données des marques (le site republie des informations et des images collectées sur des sites tiers). → **Fait le 18 septembre 2026, sauf deux points** : (1) l'identité de l'éditeur et de l'hébergeur est à renseigner dans `apps/web/src/content/editeur.ts` — tous les champs sont `null`, les pages affichent « à renseigner » et un bandeau « version de travail » ; (2) le bandeau de consentement arrive **avec** PostHog (T7.3), il n'y a aujourd'hui aucun traceur à bloquer. Export `GET /api/v1/me/export`, suppression `DELETE /api/v1/me`, mention d'origine sur chaque fiche. Détail : `docs/JOURNAL.md`.
...
- [x] **T8.5** — **Autorisation, signature, cas d'échec et portail client faits.** Trois défauts fermés le 1er septembre 2026, les tests et le portail le 18 septembre : 8 tests d'intégration sur la caisse et le portail (refus avant tout appel à Stripe, prix lu en base, client créé une fois), bouton « Gérer mon abonnement » dans le Studio, et `GET /api/v1/plans` — la page affichait des tarifs écrits en dur pendant que la caisse lisait la base. Trouvé en chemin : **une connexion rejetée pour jeton anti-CSRF passait pour réussie** dans les quatre écrans (`lib/connexion.ts`). Détail : `docs/JOURNAL.md`. Les trois défauts d'origine :
...
| 2026-09-01 | Phases 0 (sauf T0.7), 1 et 2 complètes ; T3.9, T3.11, T3.15, T3.18, T3.24, T4.13 ; T3.2 et T3.13 partiels | **Trois découvertes.** (1) La base PostgreSQL n'existe plus : ni Docker, ni Postgres, ni dump, ni répertoire de données. Les ~40 000 produits sont perdus sauf copie ailleurs. (2) `origin/main` (`7878ad4`, 17 fév.) était **en avance** sur le disque de 11 routes et 9 fichiers — l'audit d'août portait sur une copie périmée ; `main` a été réaligné dessus et les correctifs réappliqués sur la vraie base. (3) L'environnement est cassé : `pnpm` absent, Docker absent, Node en v26. **Bloquant pour la phase 2.** Autres faits : un verrou `.git/index.lock` périmé du 30 août bloquait toute écriture git ; le dépôt GitHub est **public** ; aucun `.env` réel n'a jamais été committé ; `apps/api/.env` et `apps/web/.env` sont des liens symboliques vers le `.env` racine. **Phase 2 faite dans la foulée** : Node 22 + pnpm 9.1.0 + PostgreSQL 16.15 (Homebrew, pas Docker) réinstallés, migration `init` régénérée (l'ancienne avait perdu 3 tables), index GIN trigram ajoutés, seed réparé (il référençait des paliers d'abonnement supprimés du schéma), commande unique `pnpm bootstrap`. L'application démarre et sert des marques. **Phase 1 faite ensuite** : `espace-marque` et les doublons d'inscription supprimés (2 028 lignes), 9 liens redirigés vers `/studio`, `README.md` réécrit depuis le code, `PLAN.md` et un troisième document faux (`GETTING_STARTED.md`, non listé par ce plan) archivés, `docs/SPEC-V1.md` écrit. Typecheck web : 22 → 12 erreurs. **Puis import des 903 marques**, qui a révélé trois défauts silencieux : `scripts/` absent de `pnpm-workspace.yaml` (aucun script ne fonctionnait sur une machine vierge), trois marques à nom numérique (`909`, `1083`, `1336`) rejetées à chaque import, et surtout **687 marques sur 903 sans secteur** parce que la taxonomie du seed différait de celle du front, du sitemap, du script d'import et du fichier source. Corrigé et vérifié : 0 marque sans secteur. Trou de la phase 2 rebouché au passage — les liens `.env` sont ignorés par git, donc absents d'un clone neuf ; toutes les commandes `db:*` tournent désormais depuis la racine. **Première tranche de la phase 3 ensuite** : `AdminUser` fusionné dans `User` avec un rôle (les trois tables étaient vides — le moment le moins coûteux, et il ne se représentera pas) ; fondations posées (`lib/db.ts` singleton Prisma, `lib/auth.ts`, `lib/guards.ts`, `lib/api-response.ts`, types NextAuth) ; `pnpm admin:create` remplace la route de création d'admin ouverte. **`pnpm typecheck` passe désormais sur les 7 tâches** — et on a découvert que `apps/api` et `scripts/` n'avaient jamais eu de script `typecheck` du tout : l'API cachait 4 bugs silencieux, dont une création de marque qui échouait systématiquement. ⚠️ **Rien n'est encore protégé** : les gardes existent mais aucune route ne les utilise. **Puis les 9 routes de `/api/admin/brands/*` migrées** vers Next.js derrière `requireAdmin` (Express : 92 → 82 routes). Ce sont les **premières routes authentifiées du projet**. Le passage par Next.js n'est pas un choix esthétique : le cookie de session n'est envoyé qu'en même origine, donc les appels vers `localhost:4000` n'auraient jamais pu être authentifiés. **Découverte majeure : le middleware ne s'exécutait pas du tout** — mauvais emplacement de fichier (`apps/web/middleware.ts` au lieu de `src/`) ; le constat n°2 était donc pire que décrit. `/admin` et `/studio` sont maintenant fermés côté serveur. Deux bugs corrigés en migrant : une mise à jour partielle vidait la galerie et les réseaux sociaux d'une marque, et un slug déjà pris remontait en 500. **Puis l'arbre `/api/admin/*` entièrement migré** : plus une seule route d'administration côté Express (92 → 53 routes, `index.ts` de 4 385 à 3 162 lignes). Découverte la plus lourde de la session : **cinq pages d'administration inventaient leurs données** dès que l'appel échouait — et il échouait en permanence, trois routes appelées n'ayant jamais existé. C'est de là que venait le « 39 835 produits » de `PLAN.md`. La page des abonnements présentait des entreprises réelles et nommées comme clientes payantes. Tout supprimé, routes manquantes créées sur de vraies données. **Puis l'espace marque et les routes utilisateur migrés** : le constat n°3 est fermé. L'identité circulait sous **trois** formes — `?userId=`, `?email=` et `:userId` dans le chemin, cette dernière non couverte par le critère d'origine. Trois routes B2B (ajout de label, envoi et suppression d'image) n'avaient **aucun** contrôle. Les routes utilisateur deviennent `/api/v1/me/*`. Constat n°15 fermé côté API : les statistiques valent `null` plutôt qu'un `Math.random()`. Express : 92 → **32 routes**, `index.ts` de 4 385 à **2 287 lignes**. **Constat n°4 fermé** : il y avait **deux** foyers d'injection, pas un — les outils du chat, et `GET /api/v1/products` où `?sector=` était concaténé. Plus aucun `$queryRawUnsafe` dans le projet. **Et une huitième faille, absente de l'audit, découverte et fermée** : `POST /api/auth/claim-brand` et `register` accordaient la propriété d'une marque sans aucune vérification — une requête suffisait à devenir propriétaire de n'importe laquelle des 903 marques, ce qui contournait entièrement `requireBrandOwner`. Une revendication crée désormais une demande `PENDING`. **Six des sept constats critiques sont fermés ; le n°1 est partiel** (30 routes Express restantes, presque toutes des lectures publiques). **Limitation de débit posée** (T3.21) — il n'y en avait aucune, et `/api/v1/chat` appelle un modèle payant sans authentification : n'importe qui pouvait épuiser le budget Anthropic. **Puis les trois failles de paiement fermées** : le webhook **acceptait les événements non signés** quand le secret était absent — une requête forgée suffisait à s'octroyer un abonnement Royale sans payer ; la caisse et le portail étaient ouverts à tous et permettaient d'écraser le `stripeCustomerId` d'une marque. Le prix vient désormais de la base et non du code, où il était écrit en dur sans lien avec les tarifs affichés. Express : **28 routes**, presque toutes des lectures publiques. **T3.22 et T3.23 faits.** 53 `localhost:4000` dans 43 fichiers ramenés à une source unique. Journaux structurés (pino) : le chat journalisait **le message du visiteur**, et **22 routes renvoyaient le message d'exception Prisma au client** — noms de tables et de colonnes compris. Gestionnaire d'erreurs centralisé ajouté. **Phase 4 entamée.** Deux découvertes : **le sitemap ne listait que 100 marques sur 903** (il demandait `limit=1000` à une API qui plafonne à 100 — 89 % des fiches jamais soumises à l'indexation), et **899 logos étaient des émojis préfixés par `https://`** — `new URL('https://🌬️')` est valide, les URL autorisant l'unicode dans le nom d'hôte, si bien que la validation ne signalait rien. `robots.ts` n'existait pas ; les hôtes d'images de `next.config.js` ne correspondaient à aucune image du projet. **La fiche marque est passée en Server Component** avec métadonnées, canonique, Open Graph et JSON-LD : le critère de sortie de la phase 4 est atteint sur cette page. **Secteurs et fiche produit** passés au rendu serveur ensuite. La taxonomie était encore écrite en dur **dans deux pages de plus**, et la page d'un secteur plafonnait à 100 marques alors que « Mode & Accessoires » en compte 263. Bug trouvé au passage : `materials` et `tags` étaient **doublement encodés** — le seed appliquait un encodage en chaîne à une colonne déjà `Json` — et la fiche produit plantait dessus. **Annuaire, catalogue et régions** passés au rendu serveur ensuite. La page d'une région déduisait son nom **de la première marque renvoyée** : une région sans marque s'affichait sous un nom reconstruit à partir du slug, et une région inexistante rendait une page vide au lieu d'une 404. Enfin, en regardant l'aperçu : la construction du logo était **recopiée dans 16 fichiers**, absente de deux pages, et sa forme — `new URL(websiteUrl).hostname` — fait tomber la page sur une URL invalide. Centralisée. **Métadonnées posées sur toutes les pages publiques** (T4.8) — il n'y en avait qu'une sur 45 ; les sept pages restées `'use client'` reçoivent un `layout.tsx` qui les porte. Deux pièges consignés : un `title` en chaîne simple dans un layout imbriqué **remplace le gabarit du layout racine** pour tout son sous-arbre, et sans `metadataBase` les URL Open Graph ne peuvent pas être rendues absolues. **Et la page d'accueil publique inventait ses chiffres** : quatre collections aux compteurs écrits en dur (48, 127, 84, 156 marques) alors que la base n'en contient aucune — et chacune menait vers une **404**. **Toutes les pages publiques qui chargeaient des données sont passées en Server Components** — accueil comprise. Le vrai coupable n'a jamais été le `'use client'` mais le `fetch` dans un `useEffect`. `/marques/[slug]/produits` renvoyait « Marque non trouvée » **avec un code 200** : un moteur indexait donc une page d'erreur comme une page valide. **Prochain jalon : T4.9 (ISR sur les listes), T4.10 (les `<img>` qui comptent), T4.11 (JSON-LD des listes), puis la phase 5 — qualité des données.** |Reste ouvert : T0.7 (rotation des clés Anthropic/Cloudinary/Google, sans urgence — rien n'a fuité, le projet n'a jamais été déployé et Stripe est en clés de test), et la recherche de la base sur une autre machine. |
| 2026-09-10 | Phase 4 : T4.1 à T4.4, T4.7 à T4.9, T4.11, T4.12 | **Toutes les pages publiques qui chargeaient des données sont passées en Server Components**, accueil comprise. Le coupable n'a jamais été le `'use client'` mais le `fetch` dans un `useEffect`. Découvertes en chemin : le sitemap ne listait que **100 marques sur 903** ; **899 logos étaient des émojis** préfixés par `https://` — `new URL('https://🌬️')` est valide, les URL autorisant l'unicode dans le nom d'hôte ; la page d'accueil publique annonçait **quatre collections aux compteurs inventés** menant toutes vers des 404 ; la construction du logo était **recopiée dans 16 fichiers**, sous une forme qui fait tomber la page sur une URL invalide ; `materials` et `tags` étaient **doublement encodés** ; `/marques/[slug]/produits` renvoyait une page d'erreur **avec un code 200** ; et **l'outre-mer était absent du modèle** — 13 marques importées sans région. Métadonnées : 1 → 15 pages publiques. **Et surtout : `next build` n'avait jamais fonctionné** — cinq pages échouaient sur `useSearchParams()` sans frontière `Suspense`, dont trois depuis le commit de février. Le projet n'était donc pas déployable, ce qu'aucun document ne disait. Corrigé : 991 pages générées, `pnpm start` les sert. Données structurées ajoutées ensuite : `BreadcrumbList` partout et `ItemList` sur les listes (T4.11). |
| 2026-09-11 | T4.5, T4.6, T4.10 partiel | Recherche rendue côté serveur et **pages de résultats en `noindex`** — chaque requête créerait sinon une page de contenu mince en nombre illimité. Favoris et profil vérifiés avec une vraie session ; `/profil` et `/favoris` fermés par le middleware. Images de contenu passées à `next/image`. **Découverte : la base ne contient aucune image** — 0 photo produit, 0 couverture de marque, 0 galerie sur 903 marques. Le gain de `next/image` est donc nul aujourd'hui ; le code attend le catalogue. **Phase 4 : 11 tâches sur 13. Prochain jalon : la phase 5, qualité des données.** |
| 2026-09-11 | Phase 5 : T5.1 à T5.8 | **Qualité des données, 7 tâches sur 8.** `pnpm data:audit` (lecture seule) : 899 marques publiables sur 903. Liens morts : 37 sites confirmés, désactivation prudente (3 échecs **et** 72 h), URL jamais effacée. Un seul point d'écriture pour les produits scrappés, idempotence prouvée sur une vraie boutique (passage 2 = 0 créé / 10 mis à jour, texte éditorial intact). Publication pilotée par l'audit : 10 produits en ligne avec images, 2 retirés. **Carte : 3 → 868 marques.** Enrichissement IA préparé, **non lancé** (dépense, clé T0.7). **62 tests**, les premiers du projet. Trois défauts sérieux trouvés en chemin : l'import remettait toute marque validée en `PENDING_REVIEW` à chaque `bootstrap` ; le scraper Shopify n'écrivait jamais le lien d'achat ; `--all` WooCommerce ignorait toute marque déjà importée. **Décisions en attente** : les marques `PENDING_REVIEW` servies publiquement — **tranché le 18 septembre 2026 : validation en bloc des fiches complètes** (899), les 4 restantes attendent une région ou une description ; 35 villes à corriger dans le fichier source ; feu vert T5.7. Détail : `docs/JOURNAL.md`. |
| 2026-09-16 | Phase 6 : T6.1, T6.2, T6.3, T6.4, T6.6 ; T6.5 partiel ; T3.4 | **Les premiers tests du projet, et une CI.** Vitest sur les trois paquets ; gardes d'autorisation vérifiées **par mutation** ; 30 tests d'intégration sur une base `_test` dédiée, avec un garde-fou qui refuse la base de développement ; Playwright, 9 parcours ; GitHub Actions verte au second essai. Recherche refaite de bout en bout : **`unaccent` des deux côtés** — « creme » ne trouvait pas « CRÈME BRÛLÉE » ; filtres qui s'additionnent ; double appel au montage corrigé dans cinq composants. |
| 2026-09-17 | Phase 3 close (T3.7, T3.8, T3.14) ; T8.9 avancé ; T6.5 fermée ; catalogue complet | **Express supprimé** (3 821 lignes) : chat et webhook Stripe portés sur Next, piste d'audit sur toute écriture. Deux défauts que seule la CI pouvait voir : les routes `GET` **prérendues au build** servaient l'état de la base de ce moment (`force-dynamic` partout), et l'accueil figé. **L'accueil devient un fil de produits** (T8.9, décision du propriétaire) personnalisé par des signaux gardés dans le navigateur. Collecte complète : **38 770 produits, 392 marques**, 35 000 publiés — et ce que 37 000 vraies fiches révèlent : HTML dans 4 627 noms, 172 hôtes d'images, 1 824 **traductions WooCommerce** prises pour des produits. Parcours d'édition : le Studio « enregistrait » sans rien changer (double chargement), les fiches en cache une heure ne se rafraîchissaient jamais. |
| 2026-09-18 | T7.5 (sauf identité éditeur et bandeau T7.3), T8.5, T8.1 ; décision marques ; données | **Conformité** : mentions légales, CGU, confidentialité, contact — identité de l'éditeur à `null` et affichée comme telle ; export et suppression de compte ; origine des données sur chaque fiche. Cinq liens du pied de page menaient à des 404. **Paiement** : cas d'échec testés, portail client enfin appelé, tarifs lus en base et plus en dur. **Une connexion rejetée pour CSRF passait pour réussie** dans quatre écrans. **T8.1** : la revendication avait une file sans guichet — `/admin/revendications`, seul endroit qui crée un `BrandOwner`. Dernière page d'admin à données inventées retirée. **Décision du propriétaire : 899 marques validées en bloc**, et les onze lectures publiques respectent enfin le statut. Six communes fusionnées corrigées à la source (875 placées) ; le géocodage par lieu-dit mesuré à **6 erreurs sur 12** et cantonné à des suggestions. Pied de page : « 5000+ produits » et une adresse de contact inventée remplacés par la base. |
|  |  |  |
